...
class BaseController:
    def __init__(self, arduino, base_frame, name="base_controllers"):
        ...
        self.linear_scale_correction = rospy.get_param('~linear_scale_correction')
        self.angular_scale_correction = rospy.get_param('~angular_scale_correction')

        # add topic publisher for PID tuning
        if self.debugPID:
            self.LeftEncoderPub = rospy.Publisher('LeftEncode', Int32 , queue_size=10)
            self.RightEncoderPub = rospy.Publisher('RightEncoder', Int32 , queue_size=10)
            self.LeftPidoutPub = rospy.Publisher('LeftPidout', Int32 , queue_size=10)
            self.RightPidoutPub = rospy.Publisher('RightPidoutPub', Int32 , queue_size=10)
            self.LeftVelPub = rospy.Publisher('LeftVel', Int32 , queue_size=10)
            self.RightVelPub = rospy.Publisher('RightVel', Int32 , queue_size=10)
    ...
    def setup_pid(self, pid_params):
        ...
        self.arduino.update_pid(self.left_Kp, self.left_Kd, self.left_Ki, self.left_Ko,
				self.right_Kp, self.right_Kd,self.right_Ki,self.right_Ko)
